...
module fpga_top (
	...
	output 	reg	[3:0]	led,
	output		[7:0]	lcd,
	input		[7:0]	ioa,	/* JB: Rotary encoder 1 */
	output	reg	[7:0]	iob,	/* JC: 汎用出力 */
	output	reg	[3:0]	ioc_lo,
	input	  [3:0]	ioc_hi,
	input		[7:0]	iod	/* JE: Rotary encoder 2 */
);
wire	[31:0]	pc, instr, readdata, readdata0, readdata1, readdata4, readdata6, readdata7, writedata, dataadr;
wire	[9:0]	rte1, rte2;
wire	[3:0]	byteen;
wire		reset;
...
assign	readdata	= cs0 ? readdata0 : cs1 ? readdata1 : cs4 ? readdata4 : cs6 ? readdata6 : cs7 ? readdata7 : 0;
...
/* cs4: Rotary encoder on ioa */
rotary_enc rotary_enc1 (clk_62p5mhz, reset, ioa[3:0], rte1);
assign	readdata4	= {22'h0, rte1};

/* cs6 */
assign readdata6 = {24'h0, ioc_hi, ioc_lo};
always @ (posedge clk_62p5mhz or posedge reset)
	if (reset) ioc_lo <= 0;
	else if (cs6 && memwrite) ioc_lo <= writedata[3:0];

/* cs7: Rotary encoder on iod (JE) */
rotary_enc rotary_enc2 (clk_62p5mhz, reset, iod[3:0], rte2);
assign	readdata7	= {22'h0, rte2};
...
//***********************************************************************
// Rotary Encoder module for 62.5MHz clock
//***********************************************************************
module rotary_enc (
	input clk_62p5mhz,
	input reset,
